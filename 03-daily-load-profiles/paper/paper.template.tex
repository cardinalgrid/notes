...
\subsection{Calendar}
Weekday and day type (workday, Saturday, Sunday) are taken from the local date. The a-priori special-day list consists of U.S. federal holidays and their observed days, the day after Thanksgiving, 24 and 31 December, and Super Bowl Sunday; it covers <<special_share>> of BA-days. A load-derived \emph{heating/cooling regime} is defined from the load itself: a day is a morning-peak day if its highest hour is 12:00 or earlier, which in practice is an electric-heating morning, and an afternoon/evening-peak day otherwise.

\subsection{Temperature}
One airport station is assigned to each BA, at its main load centre or the largest city of its service area (Table~\ref{tab:stations} lists them). Hourly air temperature from NOAA's Integrated Surface Database, ISD-Lite files \cite{isdlite}, is shifted to the BA's local time and aggregated to a daily mean, minimum and maximum; days with fewer than 18 observations are dropped. A temperature regime classifies each day as heating when the daily mean is below <<t_heat_f>>$^\circ$F (<<t_heat>>$^\circ$C), cooling when above <<t_cool_f>>$^\circ$F (<<t_cool>>$^\circ$C), and mild otherwise. The <<t_heat_f>>$^\circ$F threshold is the value that best separates morning-peak days from the rest across all BAs, and the best per-BA value lies between 55 and 63$^\circ$F in every BA with a material share of morning-peak days; temperature and the morning-peak flag agree on <<t_agree>> of days. These thresholds concern the \emph{shape} of the day and should not be confused with the balance-point temperature of degree-day practice, 65$^\circ$F in the United States \cite{thom1954degreedays}, at which the \emph{level} of consumption is least sensitive to temperature. Level thresholds have been estimated repeatedly, at about 65$^\circ$F for U.S. residential customers \cite{fels1986prism,engle1986weather} and at 16 to 18$^\circ$C in Europe \cite{valor2001spain,moralcarcedo2005nonlinear,bessec2008threshold}; a lower threshold for the change of shape is expected, since the morning peak requires enough heating load to outweigh the evening. Electrified heating moving winter peaks into the morning is documented at the system level by \cite{waite2020heating}.
...
Three further models test the season and the weekday signature. A \emph{fixed-season} model (S4) crosses the four meteorological seasons with the three day types and uses the previous 52 weeks. A \emph{seasonal-analog} model uses, as references, the days of the same day type within $\pm 21$ days of the same calendar date one year earlier (A1), or one and two years earlier (A2), either alone or in union with the previous two weeks (R2+A1, R2+A2). A \emph{hybrid} model adds to the recent G3 profile a per-weekday offset, the median over the previous 52 weeks of the residual $s_j - \hat{s}_j$ for days $j$ of the same weekday. Finally, \emph{regime} models split G3 by the heating/cooling regime: by the day's own load-derived regime (an oracle, since it uses the day's own peak), by the previous day's load-derived regime, by the day's own temperature regime with two classes (heating or not) or three (heating, mild, cooling), and by the previous day's temperature regime. The temperature models are causal given a temperature forecast for the day, or given only yesterday's observation in the previous-day variants. The \emph{recommended configuration} combines the findings: day type crossed with the three-class temperature regime, with the reference set of the last two weeks plus the previous year's analogs.
...
\begin{tabular}{llrr}
\toprule
Model & Reference set & MAPE & Gain (BAs $+$/$-$) \\
\midrule
<<season_rows>>
\bottomrule
\end{tabular}
...
Fig.~\ref{fig:regime} shows, on the left, the monthly share of morning-peak days for five BAs, and on the right the gain of the load-derived regime split over G3 by BA. In the Southeast and the Northwest more than 40\% of December and January days peak in the morning; in California, New York and New England almost none do. Splitting by the day's own load-derived regime reduces the shape error by <<regime_oracle>> on average and by up to <<regime_top_gain>> in <<regime_top_ba>>; the previous-day proxy gives <<regime_prev>>. The dates on which the most BAs depart from their profiles at once (Table~\ref{tab:dates}) are regime transitions, the first cold morning of autumn and the first hot afternoon of spring, rather than holidays; the highest-ranked holiday, Thanksgiving, appears at rank <<tg_rank>>.

\subsection{Temperature regime}
Fig.~\ref{fig:temperature} shows the share of morning-peak days against daily mean temperature (left) and the gains of the temperature-based splits by BA (right); Table~\ref{tab:regime} summarises the regime models. The share of morning-peak days falls from above 60\% below 40$^\circ$F to zero above 68$^\circ$F in every BA with electric heating. The two-class temperature split reduces the shape error by <<g3t_gain>>, with the interval above zero in all <<g3t_up>> BAs; the three-class split reduces it by <<g3t3_gain>>, above 5\% in <<g3t3_need>> BAs, and outperforms the load-derived oracle. The previous day's temperature is nearly as informative as the day's own: <<g3tprev_gain>> with two classes and <<g3t3prev_gain>> with three. The recommended configuration reaches <<cfg_mape>> on the days where all models exist, against <<best_base>> for the two-week three-type model, a reduction of <<best_gain>> with the interval above zero in <<best_up>> BAs; with the previous day's temperature it reaches <<bestprev_mape>>.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig7_temperature.pdf}
\caption{Left: share of morning-peak days against daily mean temperature at the reference station, for five BAs; the dashed line is the <<t_heat_f>>$^\circ$F threshold. Right: relative reduction of shape MAPE over G3 from the two-class temperature split (same day and previous day) and from the load-derived same-day regime, by BA.}
\label{fig:temperature}
\end{figure*}

\begin{table}[t]
\caption{Regime models: mean daily shape MAPE over BAs on the days where all models exist (8-week window), relative gain over G3, and number of BAs with the 95\% interval above zero.}
\label{tab:regime}
\centering
\scriptsize
\begin{tabular}{llrrr}
\toprule
Model & Regime & MAPE & Gain & BAs \\
\midrule
<<regime_rows>>
\bottomrule
\end{tabular}
...
\begin{table}[t]
\caption{Reference stations (NOAA ISD).}
\label{tab:stations}
\centering
\scriptsize
\begin{tabular}{ll|ll|ll}
\toprule
BA & Station & BA & Station & BA & Station \\
\midrule
<<station_rows>>
\bottomrule
\end{tabular}
...
The results separate three effects that are usually bundled under ``calendar''. The weekend effect is real in every BA and dominant in the urban and West-Coast systems. The weekday effect within the working week exists, is stable, and is smaller than the day-to-day noise; estimating it costs more history than it returns, whether by grouping or by a long-run offset. The seasonal effect is the largest of the three and is best handled by recency, with the previous year's analog as a complement that anticipates where the shape is going, and worst handled by fixed classes. Above all three sits the weather regime: on the days that matter most for operations, when the peak moves from the evening to the morning, no calendar model is informed, and a temperature-conditioned day type recovers more error than every calendar distinction combined. That the previous day's temperature performs almost as well as the day's own means the regime can be used without a forecast, because the regime persists from one day to the next far more often than it switches. That the largest coordinated departures from the typical shape are regime transitions rather than holidays is, for anomaly detection, the central finding: a detector that compares readings with a calendar profile will raise its loudest alarms on the first cold morning of the year.

\subsection{Recommendations}
For profile-based methods on U.S. BA load the evidence supports: (i) three day types rather than seven; (ii) a reference set of the last two weeks plus the same three calendar weeks of the previous year, with the analog dropped where the shape is known to be changing year over year; (iii) a special-day list restricted to New Year's Day, Memorial Day, Independence Day, Labor Day, Thanksgiving and the day after, Christmas Eve and Day, New Year's Eve and Super Bowl Sunday, each mapped to the Saturday or Sunday profile as in Table~\ref{tab:holidays}; and (iv) a three-class temperature regime (heating below <<t_heat_f>>$^\circ$F, cooling above <<t_cool_f>>$^\circ$F of daily mean temperature) crossed with the day type, from a forecast when available and from the previous day's observation otherwise. Combined, these reduce the shape error by <<best_gain>> relative to the plain three-type model.

\subsection{Limitations}
Only the shape is analysed; a day whose level changes without a change of shape is invisible here. One station represents each BA; for systems spanning several climates (MISO, SWPP, PJM) it is a proxy for the load centre and the temperature gains are, if anything, understated. Thresholds are stated in Fahrenheit, the unit of U.S. operational practice; the temperature thresholds are national; per-BA thresholds would fit better and generalise worse, and the sensitivity is small because the best threshold is nearly the same everywhere. The 8-week window of the model comparison is not the best window, but the ranking of calendars does not change with the window. BAs spanning several time zones report in one and their profiles are blends. The special-day list is national; state holidays and local events are not represented. Results are for aggregate BA load and need not hold at the feeder or substation level, where the weekday signature can be larger.

\section{Conclusion}
On the public record of <<n_bas>> U.S. balancing authorities, three day-type profiles estimated from recent days outperform seven weekday profiles everywhere, the length and recency of the reference history matter more than the calendar, the previous year's analog days add to recent days, and the heating/cooling regime moves the daily shape more than any holiday. A short list of special days, mapped to Saturday or Sunday profiles, covers the calendar exceptions that the data support. These choices are being adopted in the open anomaly-detection tool of the same initiative.

\section*{Data and Code Availability}
Data: EIA-930 six-month balance files \cite{eia930monitor}, prepared with \cite{guerra2026scorecard}; NOAA ISD-Lite hourly observations \cite{isdlite}. This note, its figures and tables are reproduced by \texttt{analysis.py} and \texttt{paper.py} in \url{https://github.com/cardinalgrid/notes} (code Apache-2.0, text and figures CC BY 4.0).
